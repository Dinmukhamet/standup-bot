...
func PauseActivityCommand(bot tg.BotAPI, message tg.Message) (tg.Message, error) {
	...
	if member.IsAdministrator() {
		...
		if !activity.IsActive {
			chattable.Text = "Проект уже приостановлен 🥶"
		} else {
			activity.IsActive = false
			activity.UpdatedAt = primitive.NewDateTimeFromTime(time.Now())
			coll.Update(activity)
			chattable.Text = `
				Проект приостановлен 🥶. Standup'ы не принимаются. 
			`
		}
	} else {
